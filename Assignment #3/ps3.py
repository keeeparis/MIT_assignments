...
def update_hand(hand, word):
    ...
    for i in word:
        for e in new_hand:
            if i == e and new_hand[e] >= 1:
                new_hand[e] = new_hand.get(e, 0) - 1
    return new_hand   
...
def is_valid_word(word, hand, word_list):
    ...
    if word in word_list or find_word(word, word_list):
        for i in word:
            for e in hand:
                if i in hand:
                    if i == e and hand_copy[e] > 0:
                        hand_copy[e] = hand_copy.get(e, 0) - 1
                        result = True
                        break
                    elif i == e and hand_copy[e] == 0:
                        return False
                else:
                    return False
    return result
...
def find_word(word, word_list):
    result = False
    word_n = [x for x in word]
    for i in range(len(word)):
        for j in VOWELS:
            if word[i] == '*':
                word_n[i] = j
                if ''.join(word_n) in word_list:
                    result = True
                    return result
    return result
    ...
    while n > 0:
        ...
        if user_input == '!!':
            ...
            if is_valid_word(user_input, hand, word_list):
                temp = get_word_score(user_input, n)
                total_score += temp
                print('\'' + user_input + '\'', 'earned', temp, ' points. Total:', total_score, 'points.\n')
            else:
                print('This is not a valid form. Please choose another word.\n')
...
def substitute_hand(hand, letter):
    ...
    new_letter = random.choice(rest_letters)
    new_hand[new_letter] = temp
    return new_hand
    ...
    * Returns the total score for the series of hands
    ...
    while num_of_hands > 0:
        ...
        if help_sub > 0:
            ...
            score = play_hand(hand, word_list)
        if help_re > 0:
            ans = input('Would you like to replay a hand? ')
            if ans == 'yes':
